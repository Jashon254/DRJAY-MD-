...
const mistralSystemPrompt = "You are DRJAY MD, an intelligent bot. Someone mentioned your owner. Respond with helpful and friendly assistance.";
...
const mistral = async (m, Matrix) => {
    ...
    if (isBotOrOwnerMentioned) {
        ...
        if (text === `@${botNumber}` || text === `@${ownerNumber}`) {
            await Matrix.sendMessage(m.from, { audio: { url: 'https://telegra.ph/file/b8b94202ee501c05bfef2.jpg' } }, { quoted: m });
            return;
        }
